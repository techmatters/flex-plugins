...
async function copyAndPublishFlow(oldIdsFile, newIdsFile, flowFile, update=false) {
    ...
    const keysToRemove = ['sid', 'accountSid', 'revision', 'dateCreated', 'dateUpdated', 'webhookUrl', 'url', 'links'];
    ...
    if (update) {
        newClient.studio.flows(newIds['Flow_To_Update']).update(newFlow).then(f => console.log("Done! Flow " + f.sid + " has been updated."));
    } else {
        newClient.studio.flows.create(newFlow).then(f => console.log("Done! Flow published as " + f.sid));
    }
}
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (typeof val === 'object') {
            ...
            if ((key.indexOf('id') !== -1) || (val.indexOf('WW') === 0) || (val.indexOf('TC') === 0)) {
                ...
                if (idName == null) {
                    console.log("ERROR: The id " + val + " is not present in the fromId file.");
                    process.exit(0);
                } else {
                    val = '<<' + idName + '>>';
                }
...
var argv = require('yargs')
    .usage('npm run copy_flow -- --fromIds {file} --toIds {file} [--useFlow {file}] [--update]')
    .alias('f', 'fromIds')
    .alias('t', 'toIds')
    .alias('u', 'useFlow')
