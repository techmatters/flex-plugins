...
describe('lookupTranslation', () => {
  ...
  test('returns compiled string for a key that exists in template strings', () => {
    mockGetTemplateStrings.mockReturnValue({ MyKey: 'Hello World' } as any);
    expect(lookupTranslation('MyKey')).toBe('Hello World');
  });

  test('falls back to using the code itself as template when key does not exist in template strings', () => {
  ...
  test('passes parameters to the Handlebars template', () => {
    mockGetTemplateStrings.mockReturnValue({ Greeting: 'Hello {{name}}!' } as any);
