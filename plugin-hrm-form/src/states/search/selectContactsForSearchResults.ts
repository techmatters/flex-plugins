...
const selectContactsForSearchResults = (state: RootState, taskSid: string, contactId: string): SearchContactResult => {
  const resultReferences = selectSearchStateForTask(state, taskSid, contactId)?.searchContactsResult;
  if (!resultReferences) {
    return {
      count: 0,
      contacts: [],
