...
// Action creators
export const handleSearchFormChange =
  (taskId: string) =>
  <K extends keyof t.SearchFormValues>(name: K, value: t.SearchFormValues[K]): t.SearchActionType => {
    return {
...
export const changeSearchPage =
  (taskId: string) =>
  (page: t.SearchPagesType): t.SearchActionType => ({
...
export const viewContactDetails =
  (taskId: string) =>
  (contact: SearchContact): t.SearchActionType => ({
