...
type Props = EditCaseSummaryProps &
...
const EditCaseSummary: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (!workingCopy) {
      initialiseWorkingCopy(
        task.taskSid,
        getValues() as CaseSummaryWorkingCopy
      );
    }
  });
  ...
  const form = useCreateFormFromDefinition({
    ...
    parentsPath: '',
    updateCallback: () =>
      updateWorkingCopy(task.taskSid, getValues() as CaseSummaryWorkingCopy),
    isItemEnabled: item => {
      switch (item.name) {
        case 'childIsAtRisk':
  ...
  const save = async () => {
    ...
    // });
    // console.log('updatedCase is here', updatedCase);
  ...
  const {
    added,
    addingCounsellorName,
    updated,
    updatingCounsellorName,
  } = caseItemHistory(connectedCaseState.connectedCase, counselorsHash);
...
const mapDispatchToProps = (dispatch, { task }: EditCaseSummaryProps) => {
  return {
    ...
    initialiseWorkingCopy: bindActionCreators(
      initialiseCaseSummaryWorkingCopy,
      dispatch
    ),
    updateWorkingCopy: bindActionCreators(
      updateCaseSummaryWorkingCopy,
      dispatch
    ),
    closeActions: route => {
    ...
    updateCaseAsyncAction: (caseId: Case['id'], body: Partial<Case>) => dispatch(updateCaseAsyncAction(caseId, body))
  };
};
