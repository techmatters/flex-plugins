/**
 * Copyright (C) 2021-2025 Technology Matters
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published
 * by the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see https://www.gnu.org/licenses/.
 */

import React from 'react';
import { useSelector } from 'react-redux';

import { selectCurrentDefinitionVersion } from '../../states/configuration/selectDefinitions';
import CustomSideLink from './CustomSideLink';

type Props = {};

const CustomSideLinkSet: React.FC<Props> = () => {
  const definitionVersion = useSelector(selectCurrentDefinitionVersion);
  if (!definitionVersion) return null;
  return (
    <>
      {(definitionVersion.customLinks || []).map(({ url, type, icon, label }) => (
        <CustomSideLink
          linkType={type}
          url={url}
          key={url}
          iconKey={icon}
          labelKey={label}
          showLabel={true}
          reserveSpace={false}
        />
      ))}
    </>
  );
};

export default CustomSideLinkSet;
