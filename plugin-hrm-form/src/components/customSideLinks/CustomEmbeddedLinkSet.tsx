...
const CustomEmbeddedLinkSet: React.FC<Props> = () => {
  ...
  if (currentRoute?.route !== 'custom-link' || !currentRoute?.subroute  || !currentRoute.subroute ==='custom-link') return null;

  return (
    <iframe
      src={currentRoute.subroute}
      style={{ width: '100%', height: '100%' }}
      title={`custom:${currentRoute.subroute}`}
    />
  );
};
