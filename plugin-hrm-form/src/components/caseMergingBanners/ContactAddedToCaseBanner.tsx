...
import { removeContactFromCaseAction } from './state';

const mapDispatchToProps = dispatch => {
  const bannerAyncDispatch = asyncDispatch<AnyAction>(dispatch);
  return {
    removeContactFromCase: () => bannerAyncDispatch(removeContactFromCaseAction()),
  };
};
...
const ContactAddedToCaseBanner: React.FC<Props> = ({ caseId, removeContactFromCase }) => (
  <div
    style={{
      display: 'flex',
      width: '100%',
  ...
  >
    ...
    <span style={{ color: '#0263E0', textDecoration: 'underline', cursor: 'pointer' }}>Case #{caseId}</span>
    <span onClick={removeContactFromCase} style={{ color: '#0263E0', marginLeft: 'auto', cursor: 'pointer' }}>
      Remove from Case
    </span>
  </div>
);
