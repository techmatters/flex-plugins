locals {
  ...
  helpline_base_config_hcl = read_terragrunt_config("../../helplines/${local.short_code}/helpline-base.hcl")
  helpline_base_config = local.helpline_base_config_hcl.locals

  env_config_hcl = read_terragrunt_config("../../helplines/${local.short_code}/${local.environment}/${local.stage}.hcl")
  ...
  computed_config = {
    ...
    operating_info_key = local.short_code
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "twilio/${local.short_code}/terraform.tfstate"
...
provider "aws" {
  assume_role {
    role_arn     = "arn:aws:iam::712893914485:role/tf-twilio-iac-${local.environment}"
    session_name = "tf-${local.environment}-${local.short_code}"
  }
