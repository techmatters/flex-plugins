...
const shouldOmitLoggingEvent = (AccountSid: string, payload: DebuggerPayload) => {
  if (AccountSid === stagingAccountMap.ASELO_BETA && payload.error_code === TWILIO_INVALID_TOKEN)
    return true;
  ...
  if (productionAccounts.includes(AccountSid) && payload.error_code === TWILIO_FUNCTION_ERROR_LOG)
    return true;
  ...
  if (productionAccounts.includes(AccountSid) && payload.error_code === TWILIO_FLEX_UI_ERROR)
    return true;

  return false;
};

function findCodeBySid(map: Record<string, string>, accountSid: string): string | undefined {
  return (Object.entries(map).find(([, sid]) => accountSid === sid) ?? [])[0];
}

const loggingBaseFunction = (logInfo: string) => (accountSid: string, payload: DebuggerPayload) => {
...
const logProductionEvent: LogFun = (level) => loggingBaseFunction(`[PRODUCTION ${level}]: `);
const logStagingEvent: LogFun = (level) => loggingBaseFunction(`[STAGING ${level}]: `);
const logDevelopmentEvent: LogFun = (level) => loggingBaseFunction(`[DEVELOPMENT ${level}]: `);
...
export const handler = async (event: ALBEvent): Promise<ALBResult> => {
  ...
  const { AccountSid, Level, Timestamp, Sid, Payload } = paramsToObject(params) as DebuggerEvent;
