...
import { getWorkerAttributesHandler } from '../../../src/worker/getWorkerAttributes';
...
describe('getWorkerAttributesHandler', () => {
  let mockClient: any;
          ...
          workspaces: jest.fn().mockReturnValue({
            workers: jest.fn().mockReturnValue({
              fetch: jest.fn().mockResolvedValue({
                attributes: JSON.stringify({ helpline: 'test-helpline' }),
              }),
            }),
          }),
        },
  ...
  it('should return whitelisted attributes when worker has helpline', async () => {
    const request = createMockRequest({ workerSid: TEST_WORKER_SID });
    const result = await getWorkerAttributesHandler(request, TEST_ACCOUNT_SID);

    expect(isOk(result)).toBe(true);
    if (isOk(result)) {
      expect(result.data).toEqual({ helpline: 'test-helpline' });
    }
  });
  ...
  it('should throw when worker does not have helpline attribute', async () => {
    mockClient.taskrouter.v1.workspaces.mockReturnValue({
      workers: jest.fn().mockReturnValue({
        fetch: jest.fn().mockResolvedValue({
          attributes: JSON.stringify({ full_name: 'Alice' }),
        }),
      }),
    });
  ...
  it('should use the workspace SID from configuration', async () => {
    const request = createMockRequest({ workerSid: TEST_WORKER_SID });
    await getWorkerAttributesHandler(request, TEST_ACCOUNT_SID);
